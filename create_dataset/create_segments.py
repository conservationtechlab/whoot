...
def get_paths(home_dir):
    ...
    Returns:
        wavs_file_paths (list): List of all the full paths to a wav in
                                the given directory.
    """
    wavs_file_paths = []
...
def create_segments(wav, filtered_labels, out_path, class_list):
    ...
    Args:
        ...
        filtered_labels (pd.Dataframe): The human label file reduced
                                        to only contain the rows of
                                        detections pertinent to the
                                        wav of interest.
        out_path (str): Path to directory where segment will be saved.

        class_list (str): Path to the class list that you'd like segments
                          to be created for. What the manual ID's are in
                          the human label file- will ignore everything that
                          is misspelled or unknown labels.

    Returns:
        output_rows (pd.Dataframe): The metadata now associated with the
    ...
    if filtered_labels.empty:
        print(f"filtered labels is an empty dataframe, "
...
def create_noise_segments(wav, new_buow_rows, out_path):
    ...
    Args:
        ...
        new_buow_rows (pd.Dataframe): The human labeled detection
                                      segment metadata for the given
                                      wav.
    ...
    Returns:
        all_buow_rows (pd.Dataframe): The metadata for the detection as
                                      well as the no_buow segments created
                                      from the given wav.
    """
    ...
    while num > new_sample:
        ...
        if seconds_array[random_index] == 0 and seconds_array[random_index + 3] == 0:
            ...
            segment_path = os.path.join(out_path, id)
            segment.export(segment_path, format='wav')
            new_buow_rows.loc[new_sample] = [id,
                                             call_type,
                                             segment_path,
                                             wav,
                                             duration_of_segment,
