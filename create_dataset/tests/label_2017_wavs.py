...
Example:
    ...
    $ python segment_labeled_2017_data.py /path/to/human_labels.csv \
      /path/to/directory/of/wavs/ /path/to/directory/output/ /path/to/config.yaml
...
def read_configs(config):
    ...
    with open(config, "r", encoding='utf-8') as cfg:
        configs = yaml.load(cfg, Loader=yaml.SafeLoader)

    return configs

def create_bird_segments(labels, wavs, output, config):
...
def main(labels, wavs, output, config_file):
    """Run main script

    """
    create_bird_segments(labels, wavs, output, config_file)
...
if __name__ == '__main__':
    ...
    main(args.labels, args.wavs, args.output, args.config_file)
