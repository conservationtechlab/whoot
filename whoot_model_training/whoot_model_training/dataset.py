...
class AudioDataset(DatasetDict):
    ...
    If your dataset is an AudioDataset, it can be read by the rest of the system

    Behind the scenes, this is a Apache Arrow Dataset Dict (via hf library) where
    each key is a split of the data (test/train/valid) and the value is an arrow dataset
    ...
    def validate_format(self, ds: DatasetDict):
        """Validates dataset is correctly formatted and ready to be used for training
    ...
    def get_number_species(self) -> int:
        """
            ...
            This... isn't always the case that the dataset is species only (could have calls!)
    ...
    def get_class_labels(self) -> ClassLabel:
        ...
        A common problem is when moving between datasets creating mappings between classes
        This aims to help standardize that by being able to get the classLabels for this dataset

        Returns:
            (ClassLabel): Mapping of all the names of the labels to their index. 
        """
